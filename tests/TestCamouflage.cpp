...
void run_test_camouflage() {
    ...
    std::cout << "Observation: The Gregarious Creatures should NOT flock (Gregarious behavior absent) because they cannot see each other through the camouflage accessory." << std::endl;
    ...
    env.setEyesAccessoryDistribution({{"NoEyes", 0.0}, {"WithEyes", 1.0}});  // All have eyes
    env.setEarsAccessoryDistribution({{"NoEars", 1.0}, {"WithEars", 0.0}});  // No need for ears
    env.setCamouflageAccessoryDistribution({{"NoCamouflage", 0.0}, {"WithCamouflage", 1.0}});  // All have camouflage
    env.setFinsAccessoryDistribution({{"NoFins", 1.0}, {"WithFins", 0.0}});  // No need for fins
    env.setShellAccessoryDistribution({{"NoShell", 1.0}, {"WithShell", 0.0}});  // No need for shells
