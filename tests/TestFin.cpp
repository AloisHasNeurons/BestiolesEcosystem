...
void run_test_fin() {
    ...
    std::cout << "Observation: Bestioles with fins should move significantly faster than those without fins." << std::endl;
    ...
    env.setEyesAccessoryDistribution({{"NoEyes", 1.0}, {"WithEyes", 0.0}});  // No need for eyes
    ...
    env.setFinsAccessoryDistribution({{"NoFins", 0.5}, {"WithFins", 0.5}}); // Half with fins
