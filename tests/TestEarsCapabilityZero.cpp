...
void run_test_ears_capability_zero() {
    std::cout << "Running Test Scenario: Ears (No Eyes) - Capability Zero" << std::endl;
    std::cout << "Observation: Creatures should NOT flock because their hearing capability (Gamma) is 0." << std::endl;
    std::cout << "Close the simulation window to complete the test." << std::endl;

    // 1. Configure Ears (Zero Gamma)
