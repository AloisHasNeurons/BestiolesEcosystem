...
void run_test_shell() {
  std::cout << "Running Test Scenario: Shell Accessory Effectiveness"
            << std::endl;
  std::cout << "Observation: Kamikaze Creatures equipped with Shell "
               "accessories should never die from collisions with creatures "
               "without shells and have lower speed."
            << std::endl;
  ...
  env.setEyesAccessoryDistribution(
      {{"NoEyes", 0.0},
       {"WithEyes",
        1.0}}); // All have eyes to ensure the kamikaze behavior since we want
                // to observe the results of collisions
  ...
  env.setFinsAccessoryDistribution(
      {{"NoFins", 1.0},
       {"WithFins", 0.0}}); // No fins so that the speed comparison is clear
  env.setShellAccessoryDistribution(
      {{"NoShell", 0.5}, {"WithShell", 0.5}}); // half have shells
