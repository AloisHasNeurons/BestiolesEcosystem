...
double Anticipating::steer(IBestiole& b, std::vector<IBestiole*> bestiolesList) {
  ...
  double future_distance = std::sqrt((b_future_x - future_x) * (b_future_x - future_x) +
                                     (b_future_y - future_y) * (b_future_y - future_y));
  // compute the anticipated orientation depending on this evolution of the distance
  double anticipated_orientation;
  if (future_distance < closest_distance) {
    anticipated_orientation = closest_bestiole->getOrientation() + M_PI;  // avoid potential collision by passing parallel to this bestiole
  } else {
